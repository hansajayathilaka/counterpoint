...
- Merging to `main` triggers the release pipeline: it derives the next semver version from the commits since the last release (`fix` → patch, `feat` → minor, `BREAKING CHANGE:` footer → major), tags it `vX.Y.Z`, and publishes a GitHub Release with the generated notes. `main` requires PRs, so the pipeline only pushes the tag — it never commits back to `main`; `Directory.Build.props` has no `<Version>` to update because MinVer reads it from that tag at build time. There is no manual version bump, and no committed `CHANGELOG.md` — release notes live on the GitHub Releases page.
