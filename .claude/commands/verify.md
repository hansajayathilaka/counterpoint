...
4. Confirm the app still starts (`dotnet run --project src/HardwarePos.Ui` headless smoke, or the startup integration test).
...
If everything passes, say so in one line and propose the commit message in the house format: `P1-T07: UOM conversion in base units (FR-2.4, FR-2.5)`.
