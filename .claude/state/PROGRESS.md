...
| P1-T07 | Stock ledger and balance projection | done | 2026-09-08 | 2abaee1 feat(P1-T07): stock ledger and balance projection (FR-4, DM-05) - 5/5 Done-when criteria proven (verify.sh green: 227 domain + 442 integration tests, 6 arch tests incl. new stock-ledger-family test, 54 trigger-survival tests); moving-average cost and the COGS snapshot now live in Domain.Inventory.StockLedgerMath/MovingAverageCost, shared by SqliteStockLedger.PostAsync and RebuildStockBalanceCommand so a replay reproduces the projection exactly (a genuine rounding-drift bug was caught and fixed by the sanity test written for this); startup consistency check and stock enquiry query layer (IStockEnquiry) added; no Avalonia view built for F11 - flagged for the sales-screen task |
