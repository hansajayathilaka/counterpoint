...
| P1-T06 | Barcodes and product search | in-progress | | Implementation complete: multiple barcodes/one primary (FR-2.9), Domain.Catalogue.InternalBarcodeGenerator for internal Code128-ready barcodes with a configurable digit prefix and mod-10 check digit (FR-2.10), IProductLookup (SqliteProductLookup) rewritten on Dapper joined to stock_balance (NFR-P1), IProductSearchService/SqliteProductSearchService over the FTS5 index ranked with exact-code matches first and LIMIT 50 (FR-2.11, NFR-P2), IReindexSearchCommand (delete-all + backfill), FR-2.24 duplicate detection (barcode hard-block in BarcodeMaintenanceService, similar-name+brand warning via DuplicateProductWarningException with ConfirmDuplicate override in ProductMaintenanceService). 3/5 Done-when proven by new tests (13 integration + 20 domain, all passing; verify.sh green). The two seeded-scale benchmarks (<300ms @ 20k SKUs, <500ms @ 50k SKUs) are deliberately NOT written here per this task's explicit brief - a dedicated test-engineer subagent covers those - so this is left in-progress rather than done until that harness exists and passes. |
