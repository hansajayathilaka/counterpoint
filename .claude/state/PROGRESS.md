...
| P2-T09 | Bulk breaking | done | 2026-09-14 | feat(P2-T09): bulk breaking (FR-4.9, AC-09) - all 4 Done-when proven by BulkBreakTests.cs (AC-09 hand-worked coil-to-90m pair with cost carried across bit-exact; destination moving-average cost matches hand-worked figure; declared wastage posts as DAMAGE and OUT+IN+DAMAGE conserve value exactly for evenly-dividing quantities; AC-17 owner-only both ways); verify.sh-equivalent green (344 domain/architecture tests + 684 integration tests, all passing, no schema touched); no code review round in this transcript - DAMAGE targets the source variant (not destination), documented at length in PostBulkBreakHandler's own remarks: the wastage quantity is denominated in the destination's base unit only as a header-table display figure, but an outbound movement's cost is fixed by the ledger to whatever the target variant's own current moving-average is, and only the source's average is guaranteed unperturbed and uncontaminated by unrelated stock history within the same transaction; consider carried to review: a fixed 4-decimal-place qty x cost representation cannot always reconstruct an arbitrary target bit-for-bit when BulkBreakCommand.ActualQuantity does not evenly divide the combined value, leaving a sub-scaled-unit (well under Rs 0.01) residual for awkward-fraction breaks - bounded and documented in the handler's own remarks and in IBulkBreakValueConservationQuery's SQL comment, but the test-engineer's 1,000-random-break generative test should decide whether that needs a tolerance or should generate quantities that divide evenly; no Avalonia bulk-break screen built (matches P2-T02 through P2-T08's own precedent - Application layer + service-level tests only)
