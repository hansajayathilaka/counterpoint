...
| P1-T04 | Catalogue reference data | blocked | | 5/6 entities fully done (category, brand, tax_class, supplier, customer: create/edit/deactivate/delete-guard all proven). `uom` cannot be deactivated — the `uom` table has no `active` column in any migration through P1-T01 (`Skeleton0001`), unlike `category`/`brand`/`tax_class`/`supplier`/`customer`, which all do. Per the task's explicit stop-and-report instruction for exactly this situation, no migration was added. Everything else (services, screens, seed data, two-level category enforcement, delete guards, AC-17 authorisation) is built and tested; see task report for the recommended follow-up (`uom.active` migration, mirroring `brand.active`). |
