...
| P1-T04 | Catalogue reference data | blocked | | 2df3074 feat(P1-T04): catalogue reference-data maintenance (FR-2.20, FR-2.21, FR-6) — 5/6 entities fully done (category, brand, tax_class, supplier, customer: create/edit/deactivate/delete-guard all proven, 34 new tests + 651 total green). `uom` cannot be deactivated — the `uom` table has no `active` column in any migration through P1-T01 (`Skeleton0001`), unlike `category`/`brand`/`tax_class`/`supplier`/`customer`, which all do. Per the task's explicit stop-and-report instruction for exactly this situation, no migration was added. Everything else (services, screens, seed data, two-level category enforcement, delete guards, AC-17 authorisation) is built and tested. Follow-up needed: a small migration adding `uom.active BOOLEAN NOT NULL DEFAULT 1` (mirroring `brand.active`), then wire `IUomMaintenance.DeactivateAsync/ReactivateAsync` the same way the other five entities already are. |
