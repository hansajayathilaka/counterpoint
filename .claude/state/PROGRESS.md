...
| P2-T05 | Credit notes | in-progress | 2026-09-14 | feat(P2-T05): credit notes (FR-5 store credit, FR-3 tender) - all 5 Done-when proven by a throwaway end-to-end scratch test exercising issue/partial-redeem/insufficient-balance/expiry/reconciliation through the real handlers, then removed before commit (test-engineer owns the permanent suite); verify.sh green except one pre-existing, out-of-scope format-check failure (CHARSET error on the already-committed 20260914005508_CreditNoteConstraints0008.cs migration that landed on this branch before this task started - CLAUDE.md forbids editing a committed migration, so left untouched and flagged here); no UI screen built (matches P2-T02/03/04 precedent); design decisions: credit note number travels as TenderRequest.Reference/payment.reference (documented on TenderTypes.CreditNote and ICreditNoteRedeemer), no shop-wide default expiry policy exists so CreditNoteExpiresOn is caller-supplied and optional, credit-note lookup by number/customer is unauthenticated/permissive same as IReturnableSaleLookup (no cost/margin field to protect), expiry refusal does not write status=EXPIRED (the enclosing sale transaction rolls back on refusal so a write there would never persist) |
