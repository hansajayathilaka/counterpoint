...
| P2-T11 | Reorder alerts and stock reports (interim) | in-progress | 2026-09-15 | 28b3613 feat(P2-T11): reorder alerts and stock reports (FR-4 reorder, FR-9.7) - all 4 Done-when proven (verify.sh green: 718 integration + 6 architecture + 54 trigger-survival tests); reorder list matches hand-computed low/high-stock expectation and the documented preferred-supplier algorithm (single link, most-recent-GRN among several, lowest-supplier_id tie-break); valuation total proven to tie exactly to a raw SUM(stock_balance.qty_base*cost_avg) with a deliberately non-round figure (Money is unrounded until ToScaled, so no quantisation applies at all); AC-17-style test proves a cashier is refused by the service itself; dashboard's low-stock count and the reorder list's own count proven equal against the same seeded data; all three reports proven under the 10s budget against a 500-product synthetic catalogue. New Counterpoint.Reporting project populated for the first time (IReportConnectionFactory seam so it never needs Counterpoint.Infrastructure, own DI extension decorating the owner-only valuation query in place, mirroring Counterpoint.Backup's precedent). No UI screen built (matches P2-T02-P2-T10 precedent - service/query layer only). |
